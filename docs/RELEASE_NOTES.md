...
- **Verification**: The relevant core/web/capability/skills/secrets/version suite passed 243 tests (one Starlette deprecation warning). UI diagnostics checks passed 13 tests with 8 intentional platform skips. Isolated `scripts/smoke-opencode-skills.sh /path/to/opencode` verifies exact OpenCode 1.18.30 native discovery without credentials, model calls, or real-home changes. Repeated fixtures selected different duplicate winners across global/project roots: precedence is explicitly unverified, with no inferred winner. Review regressions cover unreadable canonical files, unknown versions, tied precedence, and binary paths containing spaces.
